...
import {
  LayoutDashboard, CalendarDays, Clock, TrendingUp,
  ShieldCheck, Settings, LogOut, Stethoscope, Menu, X,
} from "lucide-react";
...
const NAV = [
  ...
  { href: "/doctor/verify", label: "Verification", icon: ShieldCheck },
];

export function DoctorNavSidebar() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-2.5 py-3 border-t border-sidebar-border space-y-1">
        <Link
          href="/settings"
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-sidebar-foreground/50 hover:text-sidebar-foreground hover:bg-white/6 transition-colors"
        >
