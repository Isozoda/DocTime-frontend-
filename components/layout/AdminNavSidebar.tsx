...
const NAV = [
  ...
  { href: "/admin/patients", label: "Patient Directory", icon: Users },
  { href: "/admin/appointments", label: "Appointments", icon: CalendarDays },
  { href: "/doctor/verify", label: "Doctor Verification", icon: ShieldAlert },
...
export function AdminNavSidebar() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-sidebar-border shrink-0">
        ...
        <span className="text-sidebar-foreground font-bold text-base tracking-tight">DocTime Admin</span>
      </div>
        ...
        {user && (
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-sidebar-foreground/45 uppercase tracking-wider">Admin</p>
            </div>
  ...
  return (
    <>
      <aside className="hidden md:flex flex-col w-64 shrink-0 min-h-screen bg-sidebar"><SidebarContent /></aside>
      ...
      {mobileOpen && (
        <>
          ...
          <aside className="fixed left-0 top-0 h-full w-64 z-50 bg-sidebar md:hidden shadow-2xl animate-slide-down"><SidebarContent /></aside>
        </>
      )}
    </>
  );
